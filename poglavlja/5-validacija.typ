...
=== _FlowGen_ спецификација

Сљедећи листинг приказује комплетну _FlowGen_ спецификацију процеса одобравања фактура:

#{
  show figure: set block(breakable: true)
  figure(
    caption: [_FlowGen_ спецификација процеса одобравања фактура],
    ```flg
    project {
...
Семантички слој _FlowGen_ система описан је у трећем поглављу. Овдје се исправност тих провјера демонстрира на конкретним примјерима невалидних модела и одговарајућих порука о грешци.
...
_FlowGen_ језик намеће правило да задатак мора бити или ручни (додијељен улози) или аутоматски, али не обоје истовремено:
...
Сви наведени примјери потврђују да семантички слој _FlowGen_ система детектује грешке у фази обраде модела, прије покретања генерисања кода. На тај начин се обезбјеђује да генерисани систем одговара искључиво валидним и конзистентним моделима.

== Анализа предности приступа

На основу спроведене студије случаја и функционалне верификације, могу се уочити сљедеће конкретне предности _FlowGen_ приступа.

Централизација процесне логике у DSL-у омогућава да се цијели процес одобравања фактура, укључујући девет стања, десет транзиција, тринаест задатака и четири улоге, дефинише у једном текстуалном фајлу. Измјена процеса (нпр. додавање новог нивоа одобравања) захтијева промјену искључиво у _FlowGen_ спецификацији, а не у десетинама генерисаних фајлова.
...
Конзистентност између модела и имплементације гарантована је механизмом генерисања. Уколико се у _FlowGen_ моделу дефинише транзиција `ManagerApprovesTransition` из `ManagerReviewState` у `FinanceReviewState` од стране улоге `Manager`, генерисана процесна машина гарантује да се та транзиција може извршити искључиво у тим условима. Не постоји могућност одступања имплементације од спецификације.
...
Ради контекстуализације _FlowGen_ приступа, у наставку је дато структурирано поређење са два алтернативна приступа: интерпретативним BPM платформама и ручном имплементацијом.
...
#figure(
  table(
    ...
    inset: 6pt,
    stroke: 0.5pt,
    table.header([*Критеријум*], [*_FlowGen_ (генеративни DSL)*], [*Camunda / Activiti*], [*Ручна имплементација*]),
  ...
  caption: [Поређење _FlowGen_ приступа са алтернативним приступима],
) <tabela-poredjenje>
...
Одсуство условних израза у транзицијама представља значајно ограничење. Тренутно, _FlowGen_ не подржава формалне услове за транзиције (нпр. `if amount > 10000 then CFOApproval`). Одлуке о гранању процеса зависе искључиво од корисничке интеракције, односно корисник одговарајуће улоге одлучује коју транзицију ће извршити. Увођење условних израза омогућило би аутоматско одлучивање на основу вриједности атрибута ентитета, чиме би се повећала аутоматизација процеса.
...
Непостојање графичког едитора модела захтијева од корисника познавање текстуалне синтаксе _FlowGen_ језика. Овај недостатак дјелимично је ублажен развојем _VSCode_ екстензије која обезбјеђује синтаксно бојење, исјечке кода и аутоматско довршавање, чиме се значајно олакшава рад са текстуалном синтаксом. Ипак, не постоји обрнути смјер, тј. не постоји могућност креирања _FlowGen_ спецификације путем графичког интерфејса. Развој графичког едитора који генерише _FlowGen_ код представља значајан правац будућег развоја.
