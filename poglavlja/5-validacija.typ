...
На слици @fig:struktura-generisanog-spring-boot-projekta приказана је структура генерисаног Spring Boot пројекта са два процеса, која одражава организацију компоненти у складу са слојевима архитектуре (ентитети, репозиторијуми, сервиси, контролери, процесна логика).

#figure(image("../slike/struktura-generisanog-spring-boot-projekta.png", width: auto), caption: [
  Структура генерисаног Spring Boot пројекта
...
#figure(
  table(
    ...
    "Генерација кода", "Комплетна (backend + frontend)", "Дјелимична (процесни слој)", "Не постоји",

    "Runtime engine", "Није потребан", "Захтијева засебну инфраструктуру", "Није потребан",

    "Динамичка измјена процеса", "Захтијева регенерацију", "Подржана", "Захтијева измјену кода",

    "Крива учења", "BPML синтакса", "BPMN + конфигурација engine-а", "Зависи од технологија",

    "Семантичка валидација", "Статичка, прије генерације", "Дјелимична", "Не постоји (мануелна)",
...
Непостојање графичког едитора модела захтијева од корисника познавање текстуалне синтаксе BPML језика. Овај недостатак дјелимично је ублажен развојем VSCode екстензије која обезбјеђује синтаксно бојење, исјечке кода и аутоматско довршавање, чиме се значајно олакшава рад са текстуалном синтаксом. Ипак, не постоји обрнути смјер, тј. не постоји могућност креирања BPML спецификације путем графичког интерфејса. Развој графичког едитора који генерише BPML код представља значајан правац будућег развоја.
